  ...
  new Set(
    COUNTRY_CODES.flatMap((code) => {
      if (englishDisplay && portugueseDisplay) {
        return [englishDisplay.of(code), portugueseDisplay.of(code)].filter(Boolean);
      }
      if (englishDisplay) {
...
export default function ContactForm({ onSuccess, showTitle = false }) {
  ...
  const handlePhoneFocus = () => {
    if (!formData.country) {
      setPhonePlaceholder("Por favor, escolha o país de origem no campo de localização");
    }
  };
    ...
    <>
      {showTitle && (
        <div className="contact-form__header">
          ...
          <p>Preencha os dados abaixo para receber nosso contato via WhatsApp.</p>
        </div>
      )}
      <form className="contact-form" onSubmit={handleSubmit} noValidate>
        ...
        <button type="submit" className="btn btn--primary btn--specialist contact-form__submit" disabled={isSubmitting}>
          Enviar
        </button>
      </form>
      ...
      {confirmationOpen && (
        <div className="contact-form__confirmation">
          <div className="contact-form__confirmation-card">
            <button className="contact-form__confirmation-close" onClick={closeConfirmation} aria-label="Fechar">
              ×
            </button>
            <p>
              Muito Obrigado Pelo interesse e tempo prestado conosco, Iremos Contactar o mais breve possível. E não se Esqueca: Criar é o começo. Potencializar é o que nos move.
            </p>
          </div>
        </div>
      )}
